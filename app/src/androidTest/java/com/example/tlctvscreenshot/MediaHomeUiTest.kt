...
class MediaHomeUiTest {
    ...
    fun homeScreenRendersDarkMediaDashboardWidgets() {
        ...
        composeRule.onNodeWithTag("action_capture_video").assertIsDisplayed().assertIsEnabled()
    ...
    fun mediaTilesAndGalleryTabsRespond() {
        enableDebugActivityPane()
        composeRule.onNodeWithTag("action_capture_video").performClick()
        composeRule.onNodeWithTag("video_capture_dialog").assertIsDisplayed()
        composeRule.onNodeWithTag("video_record_start_button").assertIsDisplayed().performClick()
        assertAnyTextExists("Recording test video", substring = true)
        assertAnyTextExists("Review and edit")
        composeRule.onNodeWithTag("video_preview_frame").assertIsDisplayed()
        composeRule.onNodeWithTag("video_timeline_strip").performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithTag("video_trim_start_handle").performScrollTo().assertIsDisplayed().performClick()
        assertAnyTextExists("Previewing start", substring = true)
        composeRule.onNodeWithTag("video_trim_end_handle").performScrollTo().assertIsDisplayed().performClick()
        assertAnyTextExists("Previewing end", substring = true)
        composeRule.onNodeWithTag("video_trim_seek_slider").performScrollTo().assertIsDisplayed()
        composeRule.onNodeWithTag("video_keep_original_button").assertIsDisplayed().performClick()
        composeRule.waitUntil(timeoutMillis = 5_000) {
            composeRule.onAllNodesWithTag("video_capture_dialog").fetchSemanticsNodes().isEmpty()
        }
        composeRule.onNodeWithTag("video_capture_dialog").assertDoesNotExist()
        ...
        composeRule.onNodeWithTag("action_cast_menu").performClick()
        composeRule.onNodeWithTag("cast_option_video").assertIsDisplayed().performClick()
